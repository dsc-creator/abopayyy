...
// Records an attempted-but-failed transaction (insufficient balance, provider
// error, etc.) so the admin dashboard's "Failed Transactions" stat is backed
// by real data instead of always reading 0. Never throws — logging failure
// shouldn't mask the original error being thrown to the client.
async function logFailedTransaction(uid, amount, type, reason) {
  await db.collection("failedTransactions").add({
    uid, amount: amount || 0, type, reason,
    timestamp: new Date().toISOString(),
  }).catch((err) => console.error("logFailedTransaction error:", err));
}

// Tracks every VTpass purchase attempt (airtime/data/bill) in one place,
// keyed by the request_id sent to VTpass, so:
//  - the admin dashboard can show the full VTU purchase pipeline without
//    scanning every user's transactions array
//  - the requery/webhook handlers below have something to look up and update
//  - a "reversed"/"failed" webhook update later can find the right wallet to refund
async function saveVtpassRequest(requestId, patch) {
  await db.collection("vtpassRequests").doc(requestId).set(
    { requestId, updatedAt: new Date().toISOString(), ...patch },
    { merge: true }
  ).catch((err) => console.error("saveVtpassRequest error:", err));
}
...
exports.initiateTransfer = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      await logFailedTransaction(uid, amount, "transfer", "Could not verify account.");
    ...
    try {
      ...
      await logFailedTransaction(uid, amount, "transfer", "Transfer failed at provider.");
...
exports.purchaseAirtime = onCall(
  ...
  async (request) => {
    ...
    await saveVtpassRequest(requestId, {
      uid, type: "airtime", network, phone, amount,
      status: "initiated", createdAt: new Date().toISOString(),
    });
    ...
    try {
      ...
      await logFailedTransaction(uid, amount, "airtime", "Could not reach VTpass.");
      await saveVtpassRequest(requestId, { status: "failed", reason: "Could not reach VTpass." });
    ...
    if (code !== "000" && code !== "099") {
      ...
      await logFailedTransaction(uid, amount, "airtime", vtpassRes?.response_description || "VTpass non-success code.");
      await saveVtpassRequest(requestId, { status: "failed", reason: vtpassRes?.response_description || "VTpass non-success code." });
    ...
    await saveVtpassRequest(requestId, {
      ...
      reference: ref,
      vtpassTransactionId: vtpassRes?.content?.transactions?.transactionId || null,
      debited: true,
    });
...
exports.purchaseData = onCall(
  ...
  async (request) => {
    ...
    await saveVtpassRequest(requestId, {
      uid, type: "data", network, phone, variationCode, amount,
      status: "initiated", createdAt: new Date().toISOString(),
    });
    ...
    try {
      ...
      await logFailedTransaction(uid, amount, "data", "Could not reach VTpass.");
      await saveVtpassRequest(requestId, { status: "failed", reason: "Could not reach VTpass." });
    ...
    if (code !== "000" && code !== "099") {
      ...
      await logFailedTransaction(uid, amount, "data", vtpassRes?.response_description || "VTpass non-success code.");
      await saveVtpassRequest(requestId, { status: "failed", reason: vtpassRes?.response_description || "VTpass non-success code." });
...
exports.payBill = onCall(
  ...
  async (request) => {
    ...
    await saveVtpassRequest(requestId, {
      uid, type: "bill", billType, provider, billersCode, amount,
      status: "initiated", createdAt: new Date().toISOString(),
    });
    ...
    try {
      ...
      await logFailedTransaction(uid, amount, "bill", "Could not reach VTpass.");
      await saveVtpassRequest(requestId, { status: "failed", reason: "Could not reach VTpass." });
    ...
    if (code !== "000" && code !== "099") {
      ...
      await logFailedTransaction(uid, amount, "bill", vtpassRes?.response_description || "VTpass non-success code.");
      await saveVtpassRequest(requestId, { status: "failed", reason: vtpassRes?.response_description || "VTpass non-success code." });
...
// ── logLogin — records a login-log entry for admin auditing ────────────────
// Called (fire-and-forget) from AuthContext right after a successful
// email/password or Google sign-in, including through the admin login
// screen since it reuses the same AuthContext.login(). Reads IP/user-agent
// off the raw request so the client can't spoof them.
exports.logLogin = onCall(async (request) => {
  ...
  const ip = (Array.isArray(forwardedFor) ? forwardedFor[0] : forwardedFor || rawReq?.ip || "")
    .toString()
    .split(",")[0]
    .trim();
...
// ── vtpassWebhook — receives async status updates from VTpass ──────────────
// VTpass calls this when a transaction that was stuck on "pending" gets
// resolved (delivered or reversed) after the initial /pay response — see
// https://vtpass.com/documentation/transaction-update-webhook-api/.
// Register this URL in the VTpass merchant dashboard (Settings > Webhook).
// There's no signature/secret on VTpass's webhook payload (confirmed in
// their docs), so this can't cryptographically verify the sender — it's
// deliberately conservative: it only ever refunds a wallet, and only once,
// looked up by a requestId *we* generated and already have on file.
exports.vtpassWebhook = onRequest({ region: "us-central1" }, async (req, res) => {
  try {
    const { type, data } = req.body || {};
    if (type !== "transaction-update" || !data?.requestId) {
      return res.status(200).json({ response: "success" }); // ack anyway, nothing to do
    }

    const requestId = data.requestId;
    const status = data.content?.transactions?.status || (data.code === "040" ? "reversed" : null);

    const reqRef = db.collection("vtpassRequests").doc(requestId);
    const reqSnap = await reqRef.get();
    if (!reqSnap.exists) {
      console.warn("vtpassWebhook: unknown requestId", requestId);
      return res.status(200).json({ response: "success" });
    }
    const reqData = reqSnap.data();

    // Refund the wallet if the wallet was actually debited for this request
    // and it later turned out to have failed/reversed — and only once.
    const isFailureUpdate = status === "reversed" || status === "failed";
    if (isFailureUpdate && reqData.debited && !reqData.refunded) {
      await creditWallet(
        reqData.uid,
        reqData.amount,
        "RVSL-" + requestId,
        `Reversal — ${reqData.type} purchase`,
        "↩️",
        { originalReference: reqData.reference, vtpassStatus: status }
      );
      await db.collection("systemLogs").add({
        source: "vtpassWebhook",
        level: "warn",
        message: `Refunded ₦${reqData.amount} to ${reqData.uid} — VTpass reported "${status}" for ${requestId}`,
        timestamp: new Date().toISOString(),
      });
    }
    ...
    res.status(200).json({ response: "success" }); // required exact shape — VTpass retries 5x otherwise
  } catch (err) {
    console.error("vtpassWebhook error:", err);
    // Still ack — we don't want VTpass hammering retries over our bug; the
    // error is logged above and the admin System Logs page will show it if
    // it's wired into systemLogs (add that if this starts failing often).
